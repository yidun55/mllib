...
//load Data set for training model
    ...
    var data_0827 = sc.textFile("/data/mllib/skb_test_0827").filter{
        ...
        if(fields(fields.length-1).toDouble==0){
            var label = -1.toDouble
        }else{
            var label = 1.toDouble
        }
        var indices = range(0, vfields.length)
    ...
    var data_union = data_0827.union(data_p_fi.sample(true, 3))
...
//===================================================================
    ...
    var testErr = scoreAndLabels.filter(r =>r._2==1).count.toDouble/data_0901_LP.count()
    println("pred positive rate = " + testErr+"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$4")
    var true_positive = scoreAndLabels.filter(r=>r._1==r._2 &&r._2==1).count.toDouble/scoreAndLabels.filter(r=>r._1==1).count
    ...
    var p_n = scoreAndLabels.filter(r=>r._1==1).count.toDouble/scoreAndLabels.filter(r=>r._1==0).count
    println("1/0 = "+p_n +"**************************************************************")
