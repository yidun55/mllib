...
object FengKongTest {
      def  main(args: Array[String]) {
        val sparkConf = new SparkConf().setAppName("FengKongTest")
        val sc = new SparkContext(sparkConf)
        var card: RDD[(VertexId, String)]  = sc.textFile("/user/hive/warehouse/zhengxin.db/fengkong_card/000000_0",1000).map { line =>
        ...
        var card_rela: RDD[Edge[String]] = sc.textFile("/user/hive/warehouse/zhengxin.db/fengkong_rela_card_c/000000_0",1000).map { line =>
