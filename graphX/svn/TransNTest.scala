 ...
 */
object TransNTest extends App {
    val sparkConf = new SparkConf().setAppName("graph")
    val sc = new SparkContext(sparkConf)
    
    
  var vertice: RDD[(VertexId, String)]  = sc.textFile("/tmp/v.txt").map { line =>
    ...
    var edges = sc.textFile("/tmp/e.txt").map { line =>
          val fields = line.split(",")
          try{
      ...
      // Define a default user
       ...
       def makeSendStr(a:String) :String={
        ...
        graph = graph.joinVertices(result)((id, oldStr, newStr) => mergeVertice(oldStr,newStr) )
        println("***************"+times)
        graph.vertices.saveAsTextFile("/root/result37")
        //graph.edges.collect.foreach(println(_))
        //graph.vertices.collect.foreach(println(_))
