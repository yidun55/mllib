 ...
 */
@BeanInfo
case class MyBean(id: Long,card :String,code : String ,ot : String)
object Trans extends App {
  ...
  var vertice: RDD[(VertexId, MyBean)]  = sc.textFile("/data/hive/vmerge/*",50).map { line =>
          val fields = line.split(",")
          try{  
                (fields(0).toLong, MyBean(fields(0).toLong,fields(1),fields(2),fields(3)))
                // (fields(0).toLong, fields(0)+":"+fields(1))
           }catch{
              case  e:Exception =>{ (0L,MyBean(0L,"","",""))}
           }    
        }
